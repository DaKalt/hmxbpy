...
def spec_Plotting(Plot_, AllData_, product_dir, part, rebin, rescale_params,
    ...
    if not separate:
        ...
        for xi in range(1, AllData_.nGroups + 1):
            if xi == visible:
                ...
                # Plot_.addCommand(f'Marker {5 + xi} on {2 * xi - 1}')
                # Plot_.addCommand(f'lw 5 on
                # {2 * xi + AllData_.nGroups}')
            else:
                # continue
                # change linewidth of Model prediction
    ...
    # Plot_.addCommand(r'lw 2 on 2') #change linewidth only for the
    # Model/data (not sure if 2 corresponds to Model or data, need to
    # try)
    # Plot_.addCommand(r'la x Energy (keV)')
    Plot_.addCommand('wi 2')
    Plot_.addCommand(r'la y \gx')
    # Plot_.addCommand(r'LAB 2 COL 1 lw 1 LIN 0 100 JUS Lef') #to change
    # color of 0 line in delchi to black
    if not separate:
        for xi in range(1, AllData_.nGroups + 1):
            # Plot_.addCommand(f'lw 3 on {2 * AllData_.nGroups
            # + 2 * xi - 1}')
            # Plot_.addCommand(f'Marker {5 + xi} on
            # {2 * AllData_.nGroups + 2 * xi - 1}')
            if colors != []:
                Plot_.addCommand(f'color {colors[xi]} on '
                                 f'{2 * AllData_.nGroups + 2 * xi - 1}')
            if markers != []:
                Plot_.addCommand(f'Marker {markers[xi]} on '
                                 f'{2 * AllData_.nGroups + 2 * xi - 1}')
            continue
    else:
        off = ''
...
def spec_model(Xset_, AllModels_, AllData_, Model_, Fit_, Plot_, product_dir,
    ...
    # Part 1: Most general spectral Fit_
    # (only galactic absorption + powerlaw)
    ...
    # __________________________________________________________________
    # Part 2: Include possible variable absorption from
    # Magellanic Clouds
    if not skip_varabs:
        Xset_.openLog(f'{product_dir}/working/xspec_part2.log')
        AllData_.show()
        Model_("tbabs*tbvarabs*powerlaw")
        # galactic absorption in the direction of MC in units of
        # 10^22cm^-2
        AllModels_(1).TBabs.nH.values = absorption
        AllModels_(1).TBabs.nH.frozen = True
    ...
    # __________________________________________________________________
    # Part 3: Flux determination from Fit_ting (absorbed and unabsorbed
    # with and without variable absorption)
    ...
    powerlaw_index = [AllModels_(1).powerlaw.PhoIndex.values[0],
                      AllModels_(1).powerlaw.PhoIndex.error[0],
                      AllModels_(1).powerlaw.PhoIndex.error[1]]
    flux_absorbed = []
    for i in range(1, AllData_.nGroups + 1):
        flux_absorbed.append([AllModels_(i).cflux.lg10Flux.values[0],
                              AllModels_(i).cflux.lg10Flux.error[0],
                              AllModels_(i).cflux.lg10Flux.error[1]])
    ...
    flux_unabsorbed = []
    for i in range(1, AllData_.nGroups + 1):
        flux_unabsorbed.append([AllModels_(i).cflux.lg10Flux.values[0],
                                AllModels_(i).cflux.lg10Flux.error[0],
                                AllModels_(i).cflux.lg10Flux.error[1]])
    ...
    for i in range(AllData_.nGroups):
        table.write(f'{epoch} & {i + 1} & {round(powerlaw_index[0], 3)}$^'
                    f'{{+{round(powerlaw_index[2] - powerlaw_index[0], 3)}}}_'
                    f'{{-{round(powerlaw_index[0] - powerlaw_index[1], 3)}}}$ '
                    f'& -- & {10 ** flux_absorbed[i][0]}$^'
                    f'{{+{10 ** flux_absorbed[i][2] - 10 ** flux_absorbed[i][0]}}}'
                    f'_{{-{10 ** flux_absorbed[i][0] - 10 ** flux_absorbed[i][1]}}}'
                    f'$ & {lum(flux_unabsorbed[i][0], distance)}$^'
                    f'{{+{lum(flux_unabsorbed[i][2], distance) - lum(flux_unabsorbed[i][0], distance)}}}'
                    f'_{{-{lum(flux_unabsorbed[i][0], distance) - lum(flux_unabsorbed[i][1], distance)}}}$\\\\\n')
        table.write('& & & & & \\\\ \n')
    ...
    if not skip_varabs:
        ...
        powerlaw_index = [AllModels_(1).powerlaw.PhoIndex.values[0],
                          AllModels_(1).powerlaw.PhoIndex.error[0],
                          AllModels_(1).powerlaw.PhoIndex.error[1]]
        nH_varab = [AllModels_(1).TBvarabs.nH.values[0],
                    AllModels_(1).TBvarabs.nH.error[0],
                    AllModels_(1).TBvarabs.nH.error[1]]
        flux_absorbed = []
        for i in range(1, AllData_.nGroups + 1):
            flux_absorbed.append([AllModels_(i).cflux.lg10Flux.values[0],
                                  AllModels_(i).cflux.lg10Flux.error[0],
    ...
    if not skip_varabs:
        ...
        for i in range(AllData_.nGroups):
            table.write(f'{epoch} & {i + 1} & {round(powerlaw_index[0], 3)}$^'
                        f'{{+{round(powerlaw_index[2] - powerlaw_index[0], 3)}}}'
                        f'_{{-{round(powerlaw_index[0] - powerlaw_index[1], 3)}}}'
                        f'$ & {100. * nH_varab[0]}$^{{+{100 * (nH_varab[2] - nH_varab[0])}}}'
                        f'_{{-{100 * (nH_varab[0] - nH_varab[1])}}}$ & '
                        f'{10 ** flux_absorbed[i][0]}$^'
                        f'{{+{10 ** flux_absorbed[i][2] - 10 ** flux_absorbed[i][0]}}}'
                        f'_{{-{10 ** flux_absorbed[i][0] - 10 ** flux_absorbed[i][1]}}}'
                        f'$ & {lum(flux_unabsorbed[i][0], distance)}$^'
