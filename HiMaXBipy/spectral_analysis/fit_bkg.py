...
def create_ogip_atable(BackArray, BackSpecFile, SourceSpecFile,
                       outfilename, ilo, ihi):
    """
    Convert the PCA background model (1D array of counts per channel)
    into a additive Xspec table model.
    Parameters
    ------------
    BackSpecFile: str
    ...
    bkgspec, hdr = pyfits.getdata(BackSpecFile, header=True,
                                  extname='SPECTRUM')
    ...
    hdu0.header['LOELIMIT'] = (
        0, 'model value for energies below those tabulated')
    hdu0.header['HIELIMIT'] = (
        0, 'model value for energies above those tabulated')
    ...
    dtype1 = [('NAME', 'S12'), ('METHOD', '>i4'), ('INITIAL', '>f4'),
              ('DELTA', '>f4'), ('MINIMUM', '>f4'),
              ('BOTTOM', '>f4'), ('TOP', '>f4'), ('MAXIMUM', '>f4'),
              ('NUMBVALS', '>i4'), ('VALUE', '>f4', (3,))]
    ...
    hdu2 = pyfits.BinTableHDU(data=numpy.array(
        [(i, i+1) for i in range(0, Nchannel)], dtype=dtype2))
...
def create_spectral_files(fitter, result, src_filename):
    ...
    dtype = [('COUNTS', '>f4') if name == 'COUNTS' else (
        name, origdata.dtype.fields[name][0]) for name in origdata.dtype.names]
...
I analysed the background for many instruments, and stored mean and
 principle components. The data file tells us which instrument we deal
 with, so we load the correct file.
First guess: 
 1) PCA decomposition.
...
class Parameter(object):
    def __init__(self, modelname, name, val, min, max, hard_min=min,
                 hard_max=max):
...
class PCAModel(object):
    def __init__(self, data):
        ...
        p0 = Parameter(modelname=modelname, name='lognorm', val=1, min=-5,
...
def minimize(f, x0):
    return scipy.optimize.minimize(f, x0=x0, method='Nelder-Mead',
                                   tol=0.001, options=dict(maxiter=1000,
                                                           disp=False))
...
class PCAFitter(object):
    def __init__(self, bkg_filename):
        ...
        for folder in os.environ.get('BKGMODELDIR', '.'), \
            ...
            filename = os.path.join(folder, ('%s_%s_%d.json' %
                                    (telescope, instrument,
                                     self.ndata)).lower())
            if os.path.exists(filename):
                self.load(filename)
                return
            filename = os.path.join(folder, ('%s_%d.json' %
                                    (telescope, self.ndata)).lower())
            if os.path.exists(filename):
                self.load(filename)
                return
        raise Exception('ERROR: Could not load PCA components for this '
                        f'detector ({telescope} {instrument}, {self.ndata} '
                        'channels). Try the SingleFitter instead.')
    ...
    def load(self, filename):
        ...
        assert self.pca['hi'].shape == (nactivedata,), 'spectrum has differ' \
            f'ent number of channels: {len(self.pca["hi"])} vs self.ndata'
    ...
    def fit(self):
        ...
        # if self.cts.sum() < len(self.cts): # do not add gaussians to
