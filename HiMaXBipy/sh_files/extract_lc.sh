...
srctool eventfiles="${sources}" \
        ...
        srcreg="${main_dir}/src.reg" backreg="${main_dir}/bkg.reg" exttype="POINT" lctype="REGULAR+" lcpars="1.0" lcemin="0.2" lcemax="8" tstep=0.05 xgrid="0.5 1.0" \
...
do
srctool eventfiles="${sources}" \
        srccoord="J2000;$ra,$dec" prefix="${product_dir}/${src}_${region}_eROSITA_PAT${PAT}_TMoff_1.0s_${j}20" suffix=".fits" todo="LC LCCORR" insts="${j}" \
        srcreg="${main_dir}/src.reg" backreg="${main_dir}/bkg.reg" exttype="POINT" lctype="REGULAR+" lcpars="1.0" lcemin="0.2" lcemax="8" tstep=0.05 xgrid="0.5 1.0" \
        gtitype="GTI" psftype="2D_PSF" clobber="yes"
done
fi
