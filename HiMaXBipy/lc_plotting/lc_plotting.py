...
def plot_lc_UL_broken(hdulist, axs, logfile, mjdref, xflag, mincounts, color):
    for ax in axs:
        time = hdulist[1].data.field('TIME')
        delt = hdulist[1].data.field('TIMEDEL')
        cnts = hdulist[1].data.field('COUNTS')
...
def plot_lc_mincounts_broken(hdulist, axs, logfile, mjdref, xflag, mincounts, color):
    for ax in axs:
        time = hdulist[1].data.field('TIME')
        delt = hdulist[1].data.field('TIMEDEL')
        cnts = hdulist[1].data.field('COUNTS')
...
def format_axis_broken(fig, axs, obs_periods, pymin, pymax, ticknumber_x,
                       ticknumber_y, ncols, nrows, d, tilt, diag_color):
    despine = False
    last_row = []
    first_col = []
...
def set_spines(axs, despine):
    ...
    for ax in axs:
        ...
        if not ax.get_subplotspec().is_last_row():
            ...
            plt.setp(ax.xaxis.get_minorticklabels(), visible=True)
            plt.setp(ax.xaxis.get_minorticklines(), visible=True)
            plt.setp(ax.xaxis.get_majorticklabels(), visible=True)
            plt.setp(ax.xaxis.get_majorticklines(), visible=True)
        if despine or not ax.get_subplotspec().is_first_row():
        ...
        if despine or not ax.get_subplotspec().is_last_col():
...
def draw_diags(fig, d, tilt, diag_color, axs, despine):
    """
    Parameters
    ----------
    d: float
    ...
    for ax in axs:
        ...
        if ax.get_subplotspec().is_last_row():
            ...
            if not ax.get_subplotspec().is_last_col():
                ...
                ds += draw_diag(ax, xpos, xlen, ypos, ylen, **d_kwargs)
            if not ax.get_subplotspec().is_first_col():
                xpos = bounds[0]
                ds += draw_diag(ax, xpos, xlen, ypos, ylen, **d_kwargs)
