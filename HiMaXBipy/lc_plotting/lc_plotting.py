...
def plot_lc_UL(hdulist, ax, logfile, mjdref, xflag, mincounts, color):
    ...
    nrow = len(time)
    logfile.writelines(f'Total number of time bins in table: {nrow}\n')
    start_time = time[0]
    ...
    logfile.writelines(f'Start of 1st bin : {start_time} {start_time0} MJD: {mjds}\n')
    logfile.writelines(f'End of last bin  : {end_time} {end_time0} MJD: {mjde}\n')
    ...
    for i in range(nrow):
        ...
        if tmp > 3600.0:
           ...
           if counts < mincounts:
              ...
              uplimit.append(0)
           logfile.writelines(f'bin number, Start,end time of bin, duration of bin, counts , rate, error,fracexp, fractime, fracarea, loop start, loop end {nbin} {start_bin} {end_bin} {end_bin-start_bin} {counts} {nrate} {nrate_e**0.5/(nexp*delt[0])} {nexp} {ntime} {narea} {istart} {iend}\n')
           # start next bin
           nbin += 1
           istart = i
           iend += 1
    ...
    logfile.writelines(f'Start/end time of last bin {nbin} {start_bin} {end_bin} {end_bin-start_bin} {counts} {nrate/nexp} {nrate_e**0.5/nexp} {nexp} {istart} {iend}\n')
    # start of first bin at 0:
    xtime = xtime-xtime[0]+xtime_d[0]
    logfile.writelines(f'{xtime} {xtime_d}\n')
    mjd = np.array(xtime)/3600./24. + mjds
    mjd_d = np.array(xtime_d)/3600./24.

    logfile.writelines(f'Total counts: {tcounts} +/- {tcount_e}\n')
    logfile.writelines(f'Net counts {netcounts} +/- {netcounts_e}\n')
    logfile.writelines(f'binsize is {delt[0]}\n')
    logfile.writelines(f'Average rate: {trate} +/- {trate_ee}\n')

    logfile.writelines(f'Total exposure: {ttim}\n')
    logfile.writelines(f'Total fract. exposure: {texp}\n')
    ...
    # Plot limits
    logfile.writelines(f"Plot limits: {pxmin} {pxmax} {pymin} {pymax}\n")

    if xflag == 1:
       ax.errorbar(xtime, yrate, xerr=xtime_d, yerr=yrate_e, uplims=uplimit, linestyle='None', color=color, fmt='o')
    else:
       ax.errorbar(mjd, yrate, xerr=mjd_d, yerr=yrate_e, uplims=uplimit, linestyle='None', color=color, fmt='o')
...
def plot_lc_mincounts(hdulist, ax, logfile, mjdref, xflag, mincounts, color):
    ...
    logfile.writelines(f'Minimum number of counts is {mincounts}\n')
    for i in range(nrow):
        tmp = time0[i] - start_bin
        tcounts = tcounts + cnts[i]
        ...
        if tcounts_c > mincounts:
           ...
           xtime.append(0.5*(time0[istart]+time0[iend+1])) #without the +1 before but this way sections are directly connected #kald
    ...
    if counts > mincounts: #this was tcounts_c which should be 0 all the time at this point #kald
       logfile.writelines(f'Condition satisfied with {counts} {bkg}\n')
      
       yrate.append(nrate)
       yrate_e.append(nrate_e**0.5/(nexp*delt[0]))
       xtime.append(0.5*(time0[istart]+time0[iend])) #see above comment about connected #kald
       xtime_d.append(0.5*(time0[iend]-time0[istart]))
    else:
       logfile.writelines(f'averaging {yrate[-1]} {nrate}\n')
       ...
       xtime[-1] = (time0[istart_tmp] + time0[iend]) * 0.5 #changed to used istart_tmp #kald
       xtime_d[-1] = 0.5*(time0[iend]-time0[istart_tmp]) #taken out xerrer as a variable #kald
    
    # start of first bin at 0:
    mjd = np.array(xtime) / 3600. / 24. + start_time / 3600. / 24. + mjdref #adjusted here #kald
    xtime = xtime-xtime[0]+xtime_d[0]
    mjd_d = np.array(xtime_d)/3600./24.
    ...
    if xflag == 1:
       ax.errorbar(xtime, yrate, xerr=xtime_d, yerr=yrate_e, linestyle='None', color=color, fmt='o')
    else:
       ax.errorbar(mjd, yrate, xerr=mjd_d, yerr=yrate_e, linestyle='None', color=color, fmt='o', zorder=1)
    ...
    logfile.writelines(f'AMPL_MAX: {ampl_max}\n')
    logfile.writelines(f'Variability V = {variability}\n')
    logfile.writelines(f'AMPL_SIG: {ampl_sig}\n')
    logfile.writelines(f'AMPL_MAX conservative: {ampl_max2}\n')
    logfile.writelines(f'AMPL_SIG2: {ampl_sig2}\n')
...
def format_axis(ax, pxmin, pxmax, pymin, pymax, ticknumber_x, ticknumber_y):
    ...
    ax.tick_params(axis='x', which='major', direction='in', top='on',   pad=5, length=5)#, labelsize=10)
    ax.tick_params(axis='x', which='minor', direction='in', top='on',   length=3)#, labelsize=0)
    ax.tick_params(axis='y', which='major', direction='in', right='on', length=5)#, labelsize=10)
    ax.tick_params(axis='y', which='minor', direction='in', right='on', length=3)#, labelsize=0)
